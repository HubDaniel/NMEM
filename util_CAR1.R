...
hyper <- function(){
  ...
  sigma2_1_trueo    = sigma2_2_trueo = 0.414
  sigma2_a_1_trueo  =  0.096 
  sigma2_a_2_trueo  =  0.063
  sigma2_as_1_trueo =  -0.082
  sigma2_as_2_trueo =  -0.015
  sigma2_s_1_trueo  =  0.513
  sigma2_s_2_trueo  =  0.098
  sigma2_np_1_trueo =  14.502
  sigma2_np_2_trueo =  14 #0.219
  
  phi_trueo = 0.5
  
  beta_vp0o =  -1.105     #intercept
  beta_vp1o =   0     #genderM
  beta_vp2o =  -0.379     #raceW
  beta_vp3o =  0         #ethniHL
  beta_vp4o =  0         #vintage
  ...
  no <- 1000            # total number of subjects
...
data_generation <- function( hypers,
                             mu1_true=
                               c( -0.1039, -0.0942, -0.0763, -0.0551, -0.0400, -0.0660, 
                                  -0.0920, -0.1072, -0.1181, -0.0909, -0.0624, -0.0387,
                                  -0.0105,  0.0264,  0.0081, -0.0276, -0.0803, -0.1268,
                                  -0.1241, -0.1132, -0.0913, -0.0505, -0.0230,  0.0255,
                                  0.0663,  0.1085,  0.1467,  0.2402,  0.4854,  1.0057,  1.7536),
                             mu2_true=
                               c(  0.01040,  0.01078,  0.01069,  0.00975,  0.00795,  0.00559,  
                                   0.00292, -0.00019, -0.00380, -0.00746, -0.01057, -0.01249,
                                   -0.01270, -0.01111, -0.00791, -0.00328,  0.00260,  0.00945,  
                                   0.01703,  0.02535,  0.03458,  0.04468,  0.05549,  0.06685,  
                                   0.07847,  0.09042,  0.10241,  0.11400,  0.12503,  0.13570,  0.14632)){
  ...
  vp7 <- rgamma(n,shape = 15.5,rate = 0.5)                        # BMI
  vp8 <- rnorm(n,mean = 62,sd = 14)                               # age
  ...
  ### group 1 ###
  ###############
  RGDtemp <- Rcpp_GD_loop(n1, group1, ti, (round(length(ti)/2) : length(ti)))
  ...
  for(i in 1:length(allncumsum)){
    ...
    # generate CAR1 random error
    e_cov_mat = sigma2_1_true * (phi_true^(abs(outer(scaledtime,scaledtime,"-"))))
    e = c(e,mvrnorm(n = 1, rep(0,nrow(e_cov_mat)), Sigma=e_cov_mat))
  }
  ...
  RGDtemp <- Rcpp_GD_loop(n2, group2, ti, (round(length(ti)/2) : length(ti)))
  allt <- as.numeric(RGDtemp$allt)
  alln <- as.numeric(RGDtemp$alln)
...
ALGORITHM <- function(hypers,DATA){
  ...
  #beta_vp11_r  <- c(0) # they do not have initial EM measured value, manually add one
  ...
  t_diff <- vector("list",length(ids))
  
  for(i in 1:length(ids)){
    alltempdata <- yalld[yalld$id == ids[i] ,]
    nelem <- nrow(alltempdata)
    tempti = alltempdata$ti
    # t_j+1 - t_j
    t_diff[[i]] = abs(outer(tempti,tempti,"-"))
    # V1
    LR1 <- sigma2_np_1 * cubic(tempti)
    UL1 <- matrix(c(sigma2_a_1,sigma2_as_1,sigma2_as_1,sigma2_s_1),nrow=2)
    G1 <- as.matrix(bdiag(list(UL1,LR1)))
  
    V1[[i]] <- ZmatL[[i]] %*% G1 %*% t(ZmatL[[i]]) + sigma2*(phi^t_diff[[i]])
    
    eigenval = sort(eigen(V1[[i]])$values)
    eigenval1 = eigenval[1]
    eigenval2 = eigenval[2]
    if( (abs(eigenval1)<1e-4)&(eigenval2>1e-4) ){
      V1[[i]] = V1[[i]] + 1e-4 * diag(nrow(V1[[i]]))
    }
    
    
    # V2
    LR2 <- sigma2_np_2 * cubic(tempti)
    UL2 <- matrix(c(sigma2_a_2,sigma2_as_2,sigma2_as_2,sigma2_s_2),nrow=2)
    G2 <- as.matrix(bdiag(list(UL2,LR2)))
 
    V2[[i]] <- ZmatL[[i]] %*% G2 %*% t(ZmatL[[i]]) + sigma2*(phi^t_diff[[i]])
    
    eigenval = sort(eigen(V2[[i]])$values)
    eigenval1 = eigenval[1]
    eigenval2 = eigenval[2]
    if( (abs(eigenval1)<1e-4)&(eigenval2>1e-4) ){
      V2[[i]] = V2[[i]] + 1e-4 * diag(nrow(V2[[i]]))
    }
  ...
  while( (n_iter <= max_iter) & (cur_change > max_rel_change) ){
    ...
    w_1 = ifelse(w_1 < 1e-3, 1e-3, w_1)
    w_1 = ifelse(w_1 > 0.999, 0.999, w_1)
    ...
    while( (n_iter_inner <= max_iter_inner) & (cur_change_inner > max_rel_change_inner) ){
      ...
      optr <- try(optim(par=rep(0,9),
                        fn=lp4V2_CAR1,
                        gr=lp4_grV2_CAR1,
                        tiid_in=tiid, tiidc_in=tiidc, Y1_in=Y1, Y2_in=Y2,
                        w1_in=w_1, w2_in=w_2, G1_in=GL, G2_in=GL, Z_in=ZmatL, E_in=GL, T_in = t_diff,
                        method="L-BFGS-B",
                        upper=c(20,100,20,20,20,100,20,20,5),
                        #lower=c(-Inf,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf),
                        control=list(maxit=10000,factr=1e2)))


      if(class(optr)=="try-error"){
        optr <- optim(par=rep(0,9),
                      fn=lp4V2_CAR1,
                      gr=lp4_grV2_CAR1,
                      tiid_in=tiid, tiidc_in=tiidc, Y1_in=Y1, Y2_in=Y2,
                      w1_in=w_1, w2_in=w_2, G1_in=GL, G2_in=GL, Z_in=ZmatL, E_in=GL, T_in = t_diff,
                      method="L-BFGS-B",
                      upper=c( 20, 100, 20, 20, 20, 100, 20, 20, 5),
                      lower=c(-20,-100,-20,-20,-20,-100,-20,-20,-400),
                      control=list(maxit=10000,factr=1e2))
      }
      ...
      sigma2 <- lpev3_CAR1(tiid, tiidc, Y1_in=Y1, Y2_in=Y2, w1_in=w_1, w2_in=w_2, 
                           G1_in=G1EL, G2_in=G2EL, Z_in=ZmatL, T_in = t_diff, Eta9_in = Eta9)
    ...
    #beta_vp11_r <- c(beta_vp11_r,vp11hat)
  ...
  ALLtraj <- cbind(sigma2_r,sigma2_a_1_r,sigma2_a_2_r,
                   sigma2_s_1_r,sigma2_s_2_r,
                   sigma2_as_1_r,sigma2_as_2_r,
                   sigma2_np_1_r,sigma2_np_2_r,
                   beta_vp0_r,beta_vp1_r,beta_vp2_r,beta_vp3_r,beta_vp4_r,
                   beta_vp5_r,beta_vp6_r,beta_vp7_r,beta_vp8_r,beta_vp9_r,
                   beta_vp10_r,phi_r)
...
RESULTS <- function(hypers,DATA,EMresults,tm.all){
  ...
  #vp11hat=EMresults$vp11hat
  betashat=EMresults$betashat
  
  p1=EMresults$p1
  p2=EMresults$p2
  ALLtraj = EMresults$ALLtraj
  ...
  result <- matrix(c(sigma2_1_true,sigma2_a_1_true,sigma2_a_2_true,sigma2_as_1_true,sigma2_as_2_true,
                     sigma2_s_1_true,sigma2_s_2_true,sigma2_np_1_true,sigma2_np_2_true,phi_true,
                     ...
                     estSigma2_s1,estSigma2_s2,estSigma2_np1,estSigma2_np2,phi,
                   ...
                   dimnames = list(c("true_val","est_val"),
                                   c("sigma2","sigma2_a1","sigma2_a2","sigma2_as1","sigma2_as2",
                                     "sigma2_s1","sigma2_s2","sigma2_np1","sigma2_np2","phi",
  ...
  colnames(result)[31] <- "elapsed CPU time"
